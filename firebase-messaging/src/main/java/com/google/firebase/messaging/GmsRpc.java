...
class GmsRpc {
  ...
  Task<String> getToken(String instanceId) {
    Task<Bundle> rpcTask =
        startRpc(
            instanceId, Metadata.getDefaultSenderId(app), SCOPE_ALL, new Bundle() /* extras */);
    return extractResponseWhenComplete(rpcTask);
  }

  Task<?> deleteToken(String instanceId) {
    Bundle extras = new Bundle();
    // Server looks at both delete and X-delete so don't need to include both
    extras.putString(EXTRA_DELETE, "1");

    Task<Bundle> rpcTask =
        startRpc(instanceId, Metadata.getDefaultSenderId(app), SCOPE_ALL, extras);
    return extractResponseWhenComplete(rpcTask);
  }

  Task<?> subscribeToTopic(String instanceId, String cachedToken, String topic) {
    Bundle extras = new Bundle();
    ...
    Task<Bundle> rpcTask = startRpc(instanceId, to, scope, extras);
    return extractResponseWhenComplete(rpcTask);
  }

  Task<?> unsubscribeFromTopic(String instanceId, String cachedToken, String topic) {
    Bundle extras = new Bundle();
    // registration servlet expects this for topics
    extras.putString(EXTRA_TOPIC, TOPIC_PREFIX + topic);
    extras.putString(EXTRA_DELETE, "1");
  ...
  private Task<Bundle> startRpc(String instanceId, String to, String scope, Bundle extras) {
    setDefaultAttributesToBundle(instanceId, to, scope, extras);
  ...
  private Bundle setDefaultAttributesToBundle(
      String instanceId, String to, String scope, Bundle extras) {
    ...
    // Populate metadata
    extras.putString(PARAM_INSTANCE_ID, instanceId);
