...
import static com.google.firebase.messaging.FcmExecutors.newNetworkIOExecutor;
...
public class FirebaseMessaging {
  ...
  public Task<Void> deleteToken() {
    ...
    return fis.getId()
        .continueWithTask(
            ...
            fisTask -> {
              return gmsRpc
                  .deleteToken(fisTask.getResult())
                  .continueWith(
                      executorService,
                      unused -> {
                        getStore(context)
                            .deleteToken(getSubtype(), Metadata.getDefaultSenderId(firebaseApp));
                        return null;
                      });
            });
  }
  ...
  String blockingGetToken() throws IOException {
    ...
    Task<String> resultTask =
        fis.getId()
            .continueWithTask(
                newNetworkIOExecutor(),
                fisIdTask ->
                    requestDeduplicator.getOrStartGetTokenRequest(
                        senderId, () -> gmsRpc.getToken(fisIdTask.getResult())));

    try {
      String newToken = Tasks.await(resultTask);
      getStore(context).saveToken(getSubtype(), senderId, newToken, metadata.getAppVersionCode());
      if (cachedToken == null || !newToken.equals(cachedToken.token)) {
        invokeOnTokenRefresh(newToken);
      }
      return newToken;
    } catch (ExecutionException | InterruptedException e) {
      throw new IOException(e);
